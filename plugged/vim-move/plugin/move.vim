...
function s:MoveKey(key)
    return '<' . g:move_key_modifier . '-' . 'S' . '>' . a:key
endfunction
...
if g:move_map_keys
    ...
    execute 'vmap' '<C-j>' '<Plug>MoveBlockDown'
    " execute 'vmap' s:MoveKey('u') '<Plug>MoveBlockUp'
    execute 'vmap' '<C-k>' '<Plug>MoveBlockUp'
    ...
    execute 'nmap' '<C-j>' '<Plug>MoveLineDown'
    " execute 'nmap' s:MoveKey('l') '<Plug>MoveLineUp'
    execute 'nmap' '<C-k>' '<Plug>MoveLineUp'
